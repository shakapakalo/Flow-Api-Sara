...
os.environ.setdefault("PLAYWRIGHT_BROWSERS_PATH", "/home/runner/workspace/.cache/ms-playwright")
import shutil as _shutil
_system_chromium = (
    "/nix/store/qa9cnw4v5xkxyip6mb9kxqfq1z4x2dx1-chromium-138.0.7204.100/bin/chromium"
    if os.path.exists("/nix/store/qa9cnw4v5xkxyip6mb9kxqfq1z4x2dx1-chromium-138.0.7204.100/bin/chromium")
    else _shutil.which("chromium") or _shutil.which("chromium-browser") or ""
)
if _system_chromium:
    os.environ.setdefault("BROWSER_EXECUTABLE_PATH", _system_chromium)
...
class TokenBrowser:
    ...
    async def _execute_captcha(self, context, project_id: str, website_key: str, action: str, st_token: Optional[str] = None) -> Optional[str]:
        """在给定 context 中执行打码逻辑"""
        page = None
        try:
            if st_token:
                try:
                    ...
                    debug_logger.log_warning(f"[BrowserCaptcha] Token-{self.token_id} failed to inject session cookie: {_ce}")
            ...
            try:
                for attempt in range(max_retries):
                    try:
                        ...
                        token = await self._execute_captcha(context, project_id, website_key, action, st_token=st_token)
                        if token:
                            self._solve_count += 1
                            self._consecutive_browser_failures = 0
                            debug_logger.log_info(
...
class BrowserCaptchaService:
    ...
    async def get_token(self, project_id: str, action: str = "IMAGE_GENERATION", token_id: int = None) -> tuple[Optional[str], Union[int, str]]:
        ...
        # 尝试从数据库获取 ST，用于向 Google 注入会话 Cookie，提升 reCAPTCHA 分数
        st_token: Optional[str] = None
        if token_id is not None and self.db is not None:
            try:
                db_token = await self.db.get_token(token_id)
                if db_token and getattr(db_token, "st", None):
                    st_token = db_token.st
                    debug_logger.log_info(
                        f"[BrowserCaptcha] Token-{token_id} will inject session cookie for reCAPTCHA"
                    )
            except Exception as _e:
                debug_logger.log_warning(f"[BrowserCaptcha] Failed to fetch ST for token {token_id}: {_e}")
