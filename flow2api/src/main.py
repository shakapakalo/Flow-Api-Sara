...
def serve_html(file_path: Path, fallback: str) -> Response:
    """Serve an HTML file with no-cache headers to prevent stale browser cache."""
    if file_path.exists():
        content = file_path.read_bytes()
        return Response(content=content, media_type="text/html", headers=NO_CACHE_HEADERS)
    return HTMLResponse(content=fallback, status_code=404)


@app.get("/", response_class=HTMLResponse)
async def index():
    """Redirect to login page"""
    return serve_html(static_path / "login.html", "<h1>Flow2API</h1><p>Frontend not found</p>")
...
async def manage_page():
    ...
    return serve_html(static_path / "manage.html", "<h1>Management Page Not Found</h1>")


@app.get("/test", response_class=HTMLResponse)
async def test_page():
    """Model testing page"""
    return serve_html(static_path / "test.html", "<h1>Test Page Not Found</h1>")


@app.get("/img-test", response_class=HTMLResponse)
async def img_test_page():
    """Text-to-image test UI"""
    return serve_html(static_path / "img-test.html", "<h1>Image Test Page Not Found</h1>")
